...
lib.update = (dir, file, data, callback) => {
    fs.open(fullPath(dir, file), 'r+', (err, fileDescriptor) => {
        ...
        fs.ftruncate(fileDescriptor, (err) => {
            ...
            fs.writeFile(fileDescriptor, stringData, (err) => {
                if (err) {
                    return callback('Klaida irasinejant i turima fala');
                }
...
lib.delete = (dir, file, callback) => {
    fs.unlink(fullPath(dir, file), (err) => {
        if (err) {
            return callback('Nepavyko pasalinti failo!')
        }
        return callback(false);
    });
}
...
lib.list = (dir, callback) => {
    ...
    fs.readdir(fullFolderPath, (err, data) => {
        ...
        for (const file of data) {
            trimmedFilesNames.push(file.split('.').slice(0, -1).join('.'));
        }
        return callback(false, trimmedFilesNames);
    })
}
